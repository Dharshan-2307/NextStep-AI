...
### Decision 5: Skill Taxonomy Integration
...
### Decision 6: Platform-Wide Data Model with Reconciliation
...
**Purpose**: Validate uploaded resume files before processing.
...
**Responsibilities**:
- Multimodal resume analysis (PDF input)
- Interview question generation
- Answer evaluation with scoring
- Market skill categorization
...
class GeminiService {
  ...
  /**
   * Analyzes resume using multimodal input
   * @param {File} resumeFile - PDF/DOCX file
   * @param {string} targetRole - Target job role
   * @returns {Promise<ResumeAnalysis>}
   */
  async analyzeResume(resumeFile, targetRole) {
    // Returns structured JSON with skills, experience, projects
  }
  
  /**
   * Generates interview questions based on resume claims
   * @param {Array<string>} claimedSkills
   * @param {string} experienceLevel - 'Entry'|'Mid'|'Senior'|'Expert'
   * @param {string} targetRole
   * @returns {Promise<Array<Question>>}
   */
  async generateInterviewQuestions(claimedSkills, experienceLevel, targetRole) {
    // Returns array of questions with difficulty levels
  }
   ...
   * @param {string} answer
   * @param {string} skill - Skill being tested
   * @returns {Promise<Evaluation>}
   */
  async evaluateAnswer(question, answer, skill) {
    // Returns: { score: 0-100, feedback: string, reasoning: string }
  }
  
  /**
   * Categorizes skills by market demand
   * @param {Array<string>} skills
   * @param {string} targetRole
   * @returns {Promise<SkillCategories>}
   */
  async categorizeSkills(skills, targetRole) {
    // Returns: { mustHave: [], goodToHave: [], futureProof: [] }
  }
  ...
  /**
   * Provides fallback data when AI unavailable
   * @private
   */
  _getFallbackData(type) {
    // Returns demo data for hackathon
  }
}
```
...
**Purpose**: Manages adaptive interview experience with multiple answer methods.

**Responsibilities**:
- Dynamic question selection based on resume claims
- Support for text, code, and voice answers
- Real-time answer evaluation
- Auto-save to prevent data loss
- Webcam simulation for proctoring
- Progress tracking

**Interface**:
```javascript
class InterviewEngine {
  constructor(geminiService, storageService) {
   ...
   */
  async startInterview(userId, targetRole, claimedSkills, experienceLevel) {
    // Generates questions, initializes session
  }
   ...
   * @param {string} answer
   * @param {string} answerType - 'text'|'code'|'voice'
   * @returns {Promise<Evaluation>}
   */
  async submitAnswer(sessionId, questionId, answer, answerType) {
    // Evaluates answer, updates session
  }
  
  /**
   * Gets next question based on performance
   * @param {string} sessionId
  ...
  /**
   * Completes interview and calculates results
   * @param {string} sessionId
   * @returns {Promise<InterviewResults>}
   */
  async completeInterview(sessionId) {
    // Finalizes session, returns overall results
  }
...
**Purpose**: Compares resume claims with interview performance to create accurate skill profile.

**Responsibilities**:
- Compare claimed skills vs demonstrated skills
- Categorize skills as Verified, Over-Claimed, or Under-Demonstrated
- Calculate Claim Accuracy Score
- Generate explanations for discrepancies
- Create reconciled skill profile for roadmap

**Interface**:
```javascript
class SkillReconciler {
  /**
   * Reconciles resume claims with interview results
   * @param {ResumeAnalysis} resumeData
   * @param {InterviewResults} interviewResults
   * @returns {ReconciliationResult}
   */
  reconcile(resumeData, interviewResults) {
    // Compares and categorizes skills
  }
  
  /**
   * Categorizes individual skill
   * @param {string} skill
   * @param {boolean} claimedInResume
   * @param {number} interviewScore
   * @returns {SkillStatus}
   */
  categorizeSkill(skill, claimedInResume, interviewScore) {
    // Returns: 'Verified' | 'Over-Claimed' | 'Under-Demonstrated'
  }
  
  /**
   * Calculates claim accuracy percentage
   * @param {ReconciliationResult} result
   * @returns {number}
   */
  calculateClaimAccuracy(result) {
    // Percentage of verified skills
  }
  
  /**
   * Generates explanation for discrepancy
   * @param {string} skill
   * @param {string} claimedLevel
   * @param {number} demonstratedScore
   * @returns {string}
   */
  explainDiscrepancy(skill, claimedLevel, demonstratedScore) {
...
```javascript
interface ReconciliationResult {
  verified: Array<SkillAssessment>;
  overClaimed: Array<SkillAssessment>;
  underDemonstrated: Array<SkillAssessment>;
  claimAccuracy: number; // percentage
  reconciledProfile: Array<SkillAssessment>;
}

interface SkillAssessment {
  skill: string;
  claimedLevel: string; // from resume
  demonstratedScore: number; // from interview
  status: 'Verified' | 'Over-Claimed' | 'Under-Demonstrated';
  explanation: string;
}
```
...
**Purpose**: Calculates 0-100 readiness score with transparent reasoning.

**Responsibilities**:
- Calculate initial readiness from reconciled skills
- Update readiness as user progresses
- Calculate time-to-ready estimates
- Provide transparent score explanations
...
class ReadinessCalculator {
  /**
   * Calculates initial readiness score
   * @param {ReconciliationResult} reconciledSkills
   * @param {string} experienceLevel
   * @param {string} targetRole
   * @returns {ReadinessScore}
   */
  calculateInitialScore(reconciledSkills, experienceLevel, targetRole) {
    // Returns score with reasoning
  }
  
  /**
   * Updates readiness after progress
   * @param {string} userId
   * @param {ProgressEvent} event
   * @returns {ReadinessScore}
   */
  updateScore(userId, event) {
    // Recalculates and shows delta
  }
   ...
   */
  estimateTimeToReady(currentScore, targetScore, missingSkills) {
    // Returns hours/minutes estimate
  }
  
  /**
   * Generates transparent explanation for score
   * @param {ReadinessScore} score
   * @returns {string}
   */
  explainScore(score) {
...
interface ProgressEvent {
  type: 'interview_passed' | 'skill_learned' | 'roadmap_completed';
  skill?: string;
  score?: number;
  timestamp: Date;
}
...
### 6. Storage Service (store.js)

**Purpose**: Centralized LocalStorage management with utilities.

**Responsibilities**:
- Store and retrieve user profile
- Store resume analysis results
- Store interview sessions and results
- Store reconciled skills
...
class StorageService {
  ...
  /**
   * Handles storage quota exceeded error
   * @private
   */
  _handleQuotaExceeded() {
    // Show error, suggest clearing old data
  }
}
```
...
const STORAGE_KEYS = {
  ...
  RECONCILED_SKILLS: 'reconciled_skills',
...
interface ResumeData {
  ...
  validated: boolean; // Has user taken interview?
}
```
...
```javascript
interface ReconciledSkillsData {
  reconciliation: ReconciliationResult;
  createdAt: Date;
  usedForRoadmap: boolean;
}
```
...
window.dispatchEvent(new CustomEvent('careerpilot:resume_updated', { detail: extractionResult }));
